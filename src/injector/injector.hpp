 ...
 *  Modified to include memory accessibility checks.
 *
...
#include "gvm/gvm.hpp"
 ...
 /*
     ...
     INJECTOR_ENABLE_MEMORY_ACCESS_CHECKS
         If defined, enables runtime checks for memory accessibility before read/write operations.
         By default it is enabled. Define to 0 to disable for performance.
 */
#ifndef INJECTOR_ENABLE_MEMORY_ACCESS_CHECKS
#define INJECTOR_ENABLE_MEMORY_ACCESS_CHECKS 1
#endif
...
namespace injector
{
     ...
     */
#if INJECTOR_ENABLE_MEMORY_ACCESS_CHECKS
    ...
    static bool IsRangeAccessible(memory_pointer_tr addr, size_t size, DWORD desired_access)
    {
        if (size == 0)
            return true;
        uintptr_t start = addr.as_int();          // используем as_int() вместо приведения
        uintptr_t end = start + size;
        uintptr_t page_start = start & ~(0xFFF);
        for (uintptr_t p = page_start; p < end; p += 0x1000)
        {
            if (!IsPageAccessible((void*)p, desired_access))
                return false;
        }
        return true;
    }

    // Helper: Log an error (optional, can be customized)
    static void LogMemoryError(const char* operation, memory_pointer_tr addr, size_t size)
    {
        // By default, output to debug console. Override if needed.
        char buf[256];
        sprintf_s(buf, sizeof(buf), "[Injector] %s failed at %p (size %zu) - memory not accessible\n", operation, addr.get(), size);
        OutputDebugStringA(buf);
    }

#else
     // Dummy implementations when checks are disabled
    static bool IsRangeAccessible(memory_pointer_tr, size_t, DWORD) { return true; }
    static void LogMemoryError(const char*, memory_pointer_tr, size_t) {}
#endif
    ...
    {
        DWORD old;
        return VirtualProtect(addr.get(), size, protection, &old) != 0;
    }
    ...
    struct scoped_unprotect
    {
        ...
        bool is_ok() const { return bUnprotected; }
    };
    ...
    inline bool WriteMemoryRaw(memory_pointer_tr addr, void* value, size_t size, bool vp)
    {
        ...
        if (!accessible)
        {
            LogMemoryError("WriteMemoryRaw", addr, size);
            return false;
        }
    ...
    inline bool ReadMemoryRaw(memory_pointer_tr addr, void* ret, size_t size, bool vp)
    {
        ...
        if (!accessible)
        {
            LogMemoryError("ReadMemoryRaw", addr, size);
            return false;
        }
    ...
    inline bool MemoryFill(memory_pointer_tr addr, uint8_t value, size_t size, bool vp)
    {
        ...
        if (!accessible)
        {
            LogMemoryError("MemoryFill", addr, size);
            return false;
        }
     ...
     */
    template <class T>
    inline bool WriteObject(memory_pointer_tr addr, const T& value, bool vp = false)
    {
        return WriteMemoryRaw(addr, (void*)&value, sizeof(value), vp);
    }
     ...
     */
    template <class T>
    inline bool ReadObject(memory_pointer_tr addr, T& value, bool vp = false)
    {
        return ReadMemoryRaw(addr, &value, sizeof(value), vp);
    }
     ...
     */
    template <class T>
    inline bool WriteMemory(memory_pointer_tr addr, T value, bool vp = false)
    {
        return WriteObject(addr, value, vp);
    }
     ...
     *      Does memory unprotection if @vp is true
     *      Returns the value (zero-initialized if read fails)
     */
    template <class T>
    inline T ReadMemory(memory_pointer_tr addr, bool vp = false)
    {
        T value = T();
        ReadObject(addr, value, vp);
        return value;
    }
